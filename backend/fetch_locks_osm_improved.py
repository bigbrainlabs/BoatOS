...
Uses Overpass API - IMPROVED VERSION including waterway=lock
"""
...
def fetch_locks_from_overpass(bbox=None):
    ...
    # - waterway=lock (MAIN TAG - most common!)
    ...
    (
      node["waterway"="lock"]({bbox[0]},{bbox[1]},{bbox[2]},{bbox[3]});
      way["waterway"="lock"]({bbox[0]},{bbox[1]},{bbox[2]},{bbox[3]});
      relation["waterway"="lock"]({bbox[0]},{bbox[1]},{bbox[2]},{bbox[3]});
...
if __name__ == "__main__":
    output_file = Path(__file__).parent / "data" / "locks_osm_improved.json"

    print("OpenStreetMap Lock Fetcher (IMPROVED)")
    print("=" * 50)
